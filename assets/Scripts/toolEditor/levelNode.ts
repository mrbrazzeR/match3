...
export default class levelNode extends cc.Component {
    ...
    setFrame(fr: cc.SpriteFrame) {
        this.view.spriteFrame = fr;
        this.bubble.active = false;
        this.grass.active = false;
    }
    setBubble(isBubble: boolean) {
        this.bubble.active = isBubble;
    }
    setGrass(isGrass: boolean, fr: cc.SpriteFrame) {
        if (isGrass) {
            this.grass.active = isGrass;
            this.grass.getComponent(cc.Sprite).spriteFrame = fr;
        }
        else {
            this.grass.active = isGrass;
        }
